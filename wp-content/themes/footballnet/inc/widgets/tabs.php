...
$effects = array (
	...
	'fadeIn'=>array (
		'title'=>'Fade in',
		'action'=>"jQuery(this).addClass('active').siblings('.caption').removeClass('active').parents('.widget_tabs').find('.tab_widget').hide().removeClass('active').eq(jQuery(this).index()).fadeIn('slow');"
	),
	'slideDonw'=>array (
		'title'=>'Slide down',
		'action'=>"jQuery(this).addClass('active').siblings('.caption').removeClass('active').parents('.widget_tabs').find('.tab_widget').hide().removeClass('active').eq(jQuery(this).index()).slideDown();"
	)
);
			...
			jQuery(document).ready(function() {
				jQuery('.widget_tabs').each(function() {
					...
					jQuery('.tab_widget', this).each(function() {
						tabs.find('.tabs_captions').append(jQuery(this).find('.caption'));
					});
					tabs.find('.caption').each(function() {
						jQuery(this).html(jQuery(this).text());
					});
					tabs.find('.tab_widget:first').addClass('active');
					tabs.find('.caption:first').addClass('active');
				});
				
				jQuery('.widget_tabs .caption').die();
				jQuery('.widget_tabs .caption').live('click', function() {
